...
impl<'a> Candidate<'a> {
    /// Create a new candidate for matching from the given path. It normalizes the path for easy lookup/comparison
    pub fn new<P: AsRef<Path> + ?Sized>(path: &'a P) -> Candidate<'a> {
        let path = normalize_path(Vec::from_path_lossy(path.as_ref()));
        Candidate { path }
    }
    /// Create a new candidate for matching from the given path without normalizing
    pub fn new_raw<P: AsRef<Path> + ?Sized>(path: &'a P) -> Candidate<'a> {
        let path = Vec::from_path_lossy(path.as_ref());
        Candidate { path }
    }

    /// return the path as a Cow
    ...
    /// Strip upto `prefix_len` bytes from the path
