...
impl EventHeader {
    pub fn new(process_id: i64, parent_process_id: i64, event_type: EventType, file_path: String, child_count: u32) -> Self {
        Self { process_id, parent_process_id, event_type, file_path, child_count }
    }
...
impl DynDepTracker {
    pub fn build(root_process_id: u32, tracer: crossbeam_channel::Sender<EventHeader>) -> DynDepTracker {
        ...
        let provider_process = provider_builder.add_callback(move |event,
                                                                   schema_locator|
            {
                ...
                if op == PROC_CREATION_OPCODE { // process creation
                        ...
                        if let Ok(parent_id) = parser.try_parse::<u32>("ParentId") {
                            if let Ok(processid) = parser.try_parse::<u32>("ProcessId") {
                                ...
                                if active_processtreeids.contains(&processid) {
                                    ...
                                    numchildren_and_self.insert(cur_id, numchildren_and_self.get(&cur_id).unwrap_or(&1) + 1);
                                    ...
                                    numchildren_and_self.insert(parent_id, numchildren_and_self.get(&parent_id).unwrap_or(&1) + 1);
                                    txpar.send((cur_id, parent_id)).unwrap();
                                    ctx_process.send(EventHeader::new(cur_id, parent_id,
                                                                      EventType::ProcessCreation, "".to_string(),
                                                                      0,
                                    )).unwrap();
                                }
                            }
                ...
                } else if op == PROC_DELETION_OPCODE { // process deletion
                    if let Ok(sch) = schema_locator.event_schema(event) {
                        let parser = Parser::create(event, &sch);
                        if let Ok(processid) = parser.try_parse::<u32>("ProcessId") {
                            if active_processtreeids.remove(&processid)
                            {
                                ...
                                *g += 1;
                                ...
                                if let Some(parent_id) = parents.get(&pid) {
                                    numchildren_and_self.entry(*parent_id).and_modify(|x| *x -= 1);
                                    let child_count = numchildren_and_self[parent_id];
                                    ctx_process.send(EventHeader::new(pid, *parent_id,
                                                                      EventType::ProcessDeletion,
                                                                      "".to_string(), child_count)).unwrap();
                                }
        ...
        let provider_disc_io = provider_builder.add_callback(move |event, schema_locator| {
                ...
                if let Ok(sch) = schema_locator.event_schema(event) {
                    ...
                    if event.opcode() == 64 {
                        if let Ok(open_path) = parser.try_parse::<String>("OpenPath") {
                            let open_path_buf = if open_path.starts_with("\\Device\\HarddiskVolume") {
                                PathBuf::from(open_path.replacen("\\Device\\", "\\\\?\\", 1))
                            } else {
                                PathBuf::from(open_path.as_str())
                            };
                            let open_path = canonicalize(open_path_buf).unwrap_or(PathBuf::from(open_path));
                    ...
                    } else if event.opcode() == 68 {// for write
        ...
        DynDepTracker { process_provider: Some(provider_process), file_init_io_provider: Some(provider_disc_io), kernel_trace: None }
    }

    pub fn start_and_process(&mut self) -> Result<(), TraceError> {
        let builder = KernelTrace::new();
        let b = builder.enable(self.process_provider.take().unwrap()).enable(self.file_init_io_provider.take().unwrap());
